...
class LoginScreen extends ParentComponent {
  ...
  handleLogin = () => { // Create Form Data And Send Request
    ...
    var that = this;
    this.handleRequest ( 'https://www.matthewfrankland.co.uk/dissertation/login/login.php', form_data, function ( response ) {
      if ( response ) { // On Success Move To Logged In Tab Controllers
        const resetAction = StackActions.reset( { index: 0, actions: [ NavigationActions.navigate( { routeName: 'Tab_Navigator' } ) ] } );
  ...
  handleForgotPass = () => {
    ...
    form_data.append( 'email', this.state.email );
    var that = this;
    this.handleRequest ( 'https://www.matthewfrankland.co.uk/dissertation/resetPass/emailReset.php', form_data, function ( response ) {
      if ( response[ 'error' ] == true ) {
        Alert.alert( 'An Error Occured While Requesting Password Reset', 'Try Again Later', [ { text: 'OK' } ] );
      } else {
        if ( response ) { // If Success Returned Email Has Been Sent
