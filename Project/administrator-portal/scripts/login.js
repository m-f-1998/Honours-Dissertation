  ...
  if ( email == "" || pass == "" ) {
    ...
    if ( expression.test( email.toLowerCase() ) ) { // Test E-Mail Is Valid On Lower Case For Realism
      if ( email.endsWith( ".ac.uk" ) ) { // Must Be Education Email
        ...
        $.ajax({
          url: 'https://www.matthewfrankland.co.uk/dissertation/login/login.php',
          data: fd,
          processData: false,
          contentType: false,
          type: 'POST',
              ...
              if ( data[ "error" ] )
                ...
                if ( data[ "account" ][ 0 ][ "is_admin" ] == 1 ) {
                  if ( data[ "account" ][ 0 ][ "email_verified" ] == 1 ) {
                    ...
                    $.redirect( 'https://www.matthewfrankland.co.uk/dissertation/administrator-portal/lecturer-register/', { account: data[ "account" ][ 0 ], code: data[ "code" ] } );
                  } else {
  ...
  if ( email == "" ) {
    ...
    if ( expression.test( email.toLowerCase() ) ) { // Test E-Mail Is Valid On Lower Case For Realism
      if ( email.endsWith( ".ac.uk" ) ) { // Must Be Education Email
        ...
        fd.append( 'email', email );
        $.ajax({
          url: 'https://www.matthewfrankland.co.uk/dissertation/resetPass/emailReset.php',
          data: fd,
          processData: false,
          contentType: false,
          type: 'POST',
